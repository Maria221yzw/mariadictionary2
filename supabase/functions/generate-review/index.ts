...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body for config params
    let body: any = {};
    try { body = await req.json(); } catch { /* no body is fine */ }
    const { scenario, difficulty, scenarioPrompt, difficultyPrompt, wordIds } = body;

    // Fetch user's vocab - if wordIds provided, fetch those specifically
    ...
    // Prioritize mastery_level < 4, then fill with others (only when no specific words selected)
    let selected;
    if (wordIds && wordIds.length > 0) {
      selected = vocab;
    } else {
    ...
    const systemPrompt = `你是英语三阶段复习题生成器。当前练习配置：
- 场景：${scenarioContext}
